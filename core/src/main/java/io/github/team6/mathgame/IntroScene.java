...
public class IntroScene extends Scene {
    ...
    public void onEnter() {
        ...
        try {
            iconTime = new com.badlogic.gdx.graphics.Texture(Gdx.files.internal("powerup_time.png"));
        } catch (Exception e) {
            iconTime = null;
        }
        try {
            iconLife = new com.badlogic.gdx.graphics.Texture(Gdx.files.internal("powerup_life.png"));
        } catch (Exception e) {
            iconLife = null;
        }
        try {
            iconMultiplier = new com.badlogic.gdx.graphics.Texture(Gdx.files.internal("powerup_multiplier.png"));
        } catch (Exception e) {
            iconMultiplier = null;
        }
    ...
    public void render(SpriteBatch batch) {
        ...
        if (levelContext == 1 && currentPanel == POWERUP_PANEL_INDEX) {
            ...
            batch.begin();
            if (iconTime != null)
                batch.draw(iconTime, startX, iconY, iconSize, iconSize);
            if (iconLife != null)
                batch.draw(iconLife, startX + iconGap, iconY, iconSize, iconSize);
            if (iconMultiplier != null)
                batch.draw(iconMultiplier, startX + iconGap * 2, iconY, iconSize, iconSize);
            batch.end();
        }
    ...
    private void advance() {
        ...
        // If still typing, snap to full text first (one click to finish, next click to
        // advance)
        if (charsShown < fullBody.length()) {
